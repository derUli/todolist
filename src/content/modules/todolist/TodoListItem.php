...
class TodoListItem extends Model
{
    ...
    public static function getAllDonebyUser($user = null, $done = true)
    {
        if (! $user) {
            $user = get_user_id();
        }
        $result = array();
        $query = Database::pQuery("select id from `{prefix}todolist_items` where user_id = ? and done = ? order by position", array(
            $user,
            $done
        ), true);
        while ($row = Database::fetchObject($query)) {
